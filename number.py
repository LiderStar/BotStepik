...
async def start_message(message: Message):
    await message.answer(
        f'Всего игр сыграно: {user_status["total_games"]}\n'
        f'Игр выиграно: {user_status["wins"]}'
    )
...
async def play_message(message: Message):
    if not user_status["ingame"]:
        await message.answer(
            "Ура!\n\nЯ загадал число от 1 до 100, " "попробуй угадать!"
        )
        user_status["ingame"] = True
        user_status["secret_number"] = secret_num()
        user_status["total_games"] += 1
        user_status["attemps"] = ATTEMPTS
    else:
        await message.answer(
...
async def check_num(message: Message):
    if user_status["ingame"]:
        if int(message.text) == user_status["secret_number"]:
            await message.answer("Ура!!! Вы угадали число!\n\n" "Может, сыграем еще?")
            user_status["ingame"] = False
            user_status["total_games"] += 1
            user_status["wins"] += 1
        elif int(message.text) > user_status["secret_number"]:
            await message.answer("Мое число меньше")
            user_status["attemps"] -= 1
        elif int(message.text) < user_status["secret_number"]:
        ...
        if user_status["attemps"] == 0:
            await message.answer(
                ...
                f'было {user_status["secret_number"]}\n\nДавайте '
                f"сыграем еще?"
            )
            user_status["ingame"] = False
            user_status["total_games"] += 1
